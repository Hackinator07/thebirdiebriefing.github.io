...
// Function to extract logo URLs from HTML
function extractLogoUrls(html, baseUrl) {
  ...
  const patterns = [
    // Look for common logo class names and alt text
    /<img[^>]*(?:class="[^"]*logo[^"]*"|alt="[^"]*logo[^"]*")[^>]*src="([^"]+)"/gi,
    // Look for images in header/nav areas
    /<img[^>]*(?:class="[^"]*(?:header|nav|brand)[^"]*")[^>]*src="([^"]+)"/gi,
    // Look for images with specific dimensions (common for logos)
    /<img[^>]*(?:width="[0-9]+"|height="[0-9]+")[^>]*src="([^"]+)"/gi,
    // Look for SVG logos
    /<svg[^>]*>[\s\S]*?<\/svg>/gi
  ];
...
async function harvestLogos() {
  console.log('Starting logo harvest...\n');
  ...
  console.log('Logo harvest complete!');
}
