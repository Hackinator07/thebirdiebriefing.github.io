...
import { selectDiverseArticles } from '@/lib/articleSelection';

interface DiverseNewsProps {
  articleCount?: number;
  title?: string;
  showViewAll?: boolean;
}

export default async function DiverseNews({ 
  articleCount = 2, 
  title = "Latest News",
  showViewAll = true 
}: DiverseNewsProps) {
  const articles = await getArticles();
  const selectedArticles = selectDiverseArticles(articles, articleCount);
    ...
    <section className="bg-gray-50 rounded-lg p-6">
      ...
      <div className="space-y-4">
        {selectedArticles.map((article, index) => (
          <div key={article.id} className="border border-gray-200 rounded-lg p-4 bg-white hover:shadow-md transition-shadow">
